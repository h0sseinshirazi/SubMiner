...
struct WireFrequencyQuery {
  std::string term;
  std::optional<std::string> reading;
};

struct WireTermFrequency {
  std::string term;
  std::optional<std::string> reading;
  ...
  for (const auto& f : term.frequencies) {
    ...
    for (const auto& v : f.frequencies) {
      entry.frequencies.emplace_back(v.value, copy_lookup_string(v.display_value, budget, "frequency display value"),
                                     copy_lookup_string(v.reading, budget, "frequency reading"));
    }
...
// Query frequency dictionaries directly, including installations with no term
// dictionary. A null reading means all readings, preserving their provenance.
EMSCRIPTEN_KEEPALIVE const char* hdw_frequencies(const char* pairs_json) {
  static std::string out;
  clear_error();
  try {
    std::vector<WireFrequencyQuery> pairs;
    if (pairs_json == nullptr || glz::read<glz::opts{.error_on_unknown_keys = false}>(pairs, std::string_view{pairs_json})
        || pairs.size() > 4096) {
      throw std::runtime_error("invalid frequency query");
    }
    std::vector<WireTermFrequency> frequencies;
    LookupCopyBudget budget;
    for (const auto& pair : pairs) {
      require_lookup_text_size(pair.term, "frequency term");
      require_lookup_text_size(pair.reading.value_or(""), "frequency reading");
      std::vector<TermResult> terms(1);
      terms[0].expression = pair.term;
      terms[0].reading = pair.reading.value_or("");
      engine().query.query_freq(terms, pair.reading.has_value());
      for (const auto& group : terms[0].frequencies) {
        for (const auto& value : group.frequencies) {
          frequencies.push_back({
            copy_lookup_string(pair.term, budget, "frequency term"),
            value.reading.empty() ? std::nullopt : std::optional{copy_lookup_string(value.reading, budget, "frequency reading")},
            !value.reading.empty(),
            copy_lookup_string(group.dict_name, budget, "frequency dictionary"),
            value.value,
            value.display_value.empty() ? std::nullopt : std::optional{copy_lookup_string(value.display_value, budget, "frequency display value")},
          });
        }
      }
    }
    out = lookup_json(frequencies, budget);
  } catch (...) {
    set_error(describe_current_exception());
    out = "[]";
  }
